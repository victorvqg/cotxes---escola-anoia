...
(async () => {
  ...
  T("v4.44: en mode «Entra» la targeta és blanca (sense la classe verda «reg»)",
    !d.querySelector("#pantalla .targeta").classList.contains("reg"));
  ...
  T("v4.44: en mode «Crea el teu compte» la targeta porta la classe verda «reg»",
    d.querySelector("#pantalla .targeta").classList.contains("reg"));
  w.loginA("entra"); await tic();
  T("es pot tornar al mode Entra", pant().includes('onclick="fesLogin()"'));
  T("v4.44: … i la targeta torna a ser blanca en tornar a «Entra»",
    !d.querySelector("#pantalla .targeta").classList.contains("reg"));
  ...
  T("v4.54: l'avís de «sense conductor» reutilitza la classe .a-pend (vermell i negreta) ja existent, no un estil nou",
    /<p class="hint a-pend"[^>]*>⚠ Mia té 1 trajecte sense conductor/.test(cos()));
  w.commuta(miaViu.marca, "r13", "dl"); w.triaTab("descarrega"); await tic();
  ...
  T("v4.48: … és un avís de resposta a l'intent (es veu just després de bloquejar-lo)", d.querySelector("#avis").style.display !== "none");
  await tic(2700);
